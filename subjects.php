...
$html .= "<div class='greetings'>Hi, " . $user->get_name() . "<br><br>";
if ($user->get_type() == 'student') {
    $html .= "You are a " . $user->get_type();
    $html .= "<br><br>Student ID: " . $user->get_sID();
} else {
    $html .= "You are an " . $user->get_type();
    $html .= "<br><br>Staff ID: " . $user->get_sID();
}
$html .= "<br>Phone No.: " . $user->get_phone();
$html .= "<br>Email: " . $user->get_email();
$html .= "<br><br></div>";
// Search box feature
$html .= "
...
if ($user->get_type() == 'admin') {
    $html .= "<form action='subjects.php' method='POST'> <div class='addDisplayContainer'>
...
if (isset($_POST['prompt'])) {
    $html .= "<form action='subjects.php' method='POST'>
    <div class='wrap'>
...
if (isset($_POST['addSubject'])) {
    ...
    if (mysqli_num_rows($preResult) == 0) {
        $html .= "Lecturer is not in the database: " . $lecturer;
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $html .= "Failed to insert subject: " . $conn->error;
        }
    }
}
...
if (isset($_POST['yesChange'])) {
    ...
    $html .= $subjectCode . " has been " . $_GET['typeChange'];
} else if (isset($_POST['noChange'])) {
    $html .= "Subject removal cancelled";
}
...
function changeStatus($todo, $changeto)
{
    global $html;
    $subjectCode = $_SESSION['subjecttoChange'];
    $html .= "Are you sure you want to " . $todo . " " . $subjectCode . " ?";
    $html .= "<form action='subjects.php?typeChange=$changeto' method='POST'> 
        <div class='submitResetItem submitContainer'>
...
if ($user->get_type() != 'admin') {
    ...
    if (isset($_POST['search'])) {
        $html .= "<h4>Your search result..</h4>";
    }
    $html .= "<h2> Your Subjects</h2>";
    $html .= "<div class ='subjectList'>";

    if (isset($_POST['withdraw'])) {
        // $html .= "you press withdraw";
    ...
    if (isset($_POST['search']) && $_POST['search'] == 'search') {
            ...
            switch ($search_by) {
                case "code":
                    if ($search_input == $code) {
                        ...
                        if ($user->get_type() == 'student') {
                            $html .= "<form action='subjects.php' method='POST'> <input type='hidden' name='subjectCode' value='$code'>
        ...
        if ($stmt == null)
            $html .= "No search terms were entered into advanced search box!";
        else {
            if ($user->get_type() == 'admin') {
                $html .= "<div class ='subjectList'>";
...
// To redirect to Students page (for educator upon pressing "View Students" button)
if (isset($_POST['viewStudents'])) {
    $_SESSION['subjecttoView'] = $_SESSION['subjecttoChange'];
    $skip = true;
...
function enrollSubject($conn, $sID, $subject)
{
    ...
    if ($stmt->execute()) {
        $html .= "You are now enrolled in " . $subject . " Please refresh the page.";
    } else {
        $html .= "You are already enrolled in " . $subject;
    }
    $html .= "<br>";
}
...
function withdrawSubject($conn, $sID, $subject)
{
    ...
    // var_dump(($stmt->execute()));
    $stmt->execute();
    $html .= "You have withdrawn from " . $subject . " Please refresh the page.";
    $html .= "<br>";
...
function teachSubject($conn, $sID, $subject)
{
    ...
    // var_dump(($stmt->execute()));
    if ($stmt->execute()) {
        $html .= "You are now a lecturer for " . $subject . " Please refresh the page.";
    } else {
        $html .= "You are already a lecturer for " . $subject;
    }
    $html .= "<br>";
}
...
function notTeach($conn, $sID, $subject)
{
    ...
    // var_dump(($stmt->execute()));
    $stmt->execute();
    $html .= "You have stopped teaching " . $subject . " Please refresh the page.";
    $html .= "<br>";
...
function executeStmtDisplay($stmt)
{
    ...
    if (!$success) {
        ...
        $html .= "Error executing statement: " . $stmt->error;
        return;
    }
...
function displaySubject($conn, $result, bool $isEnrolment = false, bool $isTeaching = false)
{
    ...
    $foundResult = false;
    $html .= "<div class='buttonContainerXL'>";
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (unserialize($_SESSION['user'])->get_type() == 'admin') {
            $html .= "<form action='subjects.php' method='POST'> <div class='buttonItem'>
    <button type='submit' name='promptEdit' value='promptEdit'>Edit</button> <input type='hidden' name='subjectCode' value='$code'></div></form>";
            if ($type == 'active') {
                $html .= "<form action='subjects.php' method='POST'> <div class='buttonItem'><input type='hidden' name='subjectCode' value='$code'>
    ...
    <button type='submit' name='activate' value='activate'>Activate</button> </div></form>";
        ...
        <button type='submit' name='remove' value='remove'>Remove</button>
            ...
            $html .= "<form action='subjects.php' method='POST'> <div class='buttonItem'><input type='hidden' name='subjectCode' value='$code'>
            <button type='submit' name='enroll' value='Enroll'>Enroll</button></div></form>";
        } else if ($isTeaching && (unserialize($_SESSION['user'])->get_type() == 'educator')) {
            $html .= "<form action='subjects.php' method='POST'> <div class='buttonItem'><input type='hidden' name='subjectCode' value='$code'>
            <button type='submit' name='teach' value='Teach'>Start Teaching</button></div></form>";
        }
        $html .= "</div>";
    }
...
function displayLecturer($conn, $sID)
{
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $name = $row['name'];
        $html .= "<div class='lecturerContainer'> | " . $name . " | </div>";
    }
}
...
<!-- Remove link rel ... what is this css-file for? -->
